-- 파일 목적: 메인 업무 DB 화면 확인용 더미 데이터를 생성한다.
-- 실행 대상 DB: office_mate_2605
-- 실행 순서: 4
-- 포함 내용: 로그인/권한/계정 잠금/상태 신청/게시판/일정/채팅/프로젝트 샘플 데이터
-- 주의사항: 포트폴리오 시연과 로컬 화면 확인을 위한 더미 계정만 포함한다.
--
-- 더미 계정:
-- 아래 employee.password 값은 공통 임시 비밀번호 1111을 BCrypt로 암호화한 값이다.
